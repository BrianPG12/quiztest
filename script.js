(() => {
  ...
  function renderInsights(elements, state) {
    ...
    }).filter((row) => row.attempts >= 4).sort((a, b) => a.accuracy - b.accuracy).slice(0, 3);
  ...
  async function firestoreGet(path, idToken) {
    ...
    const raw = (_b = (_a = body.fields) == null ? void 0 : _a.payload) == null ? void 0 : _b.stringValue;
  ...
  async function setupCloudSync({
    ...
    async function pullNow() {
      ...
      const idToken = await getIdToken2();
  ...
  var projectId, FIRESTORE_BASE;
  var init_cloudSync = __esm({
    "js/features/cloudSync.js"() {
      init_syncConfig();
      init_index_esm();
      init_index_esm2();
      ({ projectId } = syncConfig.firebase);
  ...
  function createSrsManager(state) {
    function getDueRomajiList() {
      const now = Date.now();
      return Object.entries(state.srsByRomaji).filter(([, entry]) => Number(entry.dueAt || 0) <= now).sort((a, b) => Number(a[1].dueAt || 0) - Number(b[1].dueAt || 0)).map(([romaji]) => romaji);
    }
